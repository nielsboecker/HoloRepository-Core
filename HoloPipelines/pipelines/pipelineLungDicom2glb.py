...
def main(dicomPath, outputGlbPath):
	...
	generatedSegmentedLungsNiftiPath = lungSegment(generatedNiftiPath, str(pathlib.Path.cwd().joinpath("medicalScans", "nifti", "segmentedLungs")))
	generatedNumpyList = compNifti2numpy.main(str(pathlib.Path(generatedSegmentedLungsNiftiPath).joinpath("lung.nii.gz")))
	...
	generatedGlbPath = compObj2glbWrapper.main(generatedObjPath, str(pathlib.Path(outputGlbPath)), deleteOriginalObj=True, compressGlb=False)
