...
def test_main_s0_0():
	output = subprocess.check_output('python pipelineController.py -c test/testList.json s0', cwd=newCwd, shell=True).decode("utf-8")
...
def test_main_ls():
	output = subprocess.check_output('python pipelineController.py -c test/testList.json --ls', cwd=newCwd, shell=True).decode("utf-8")
	assert "dicom2glb" in str(output)

def test_main_info_0():
	output = subprocess.check_output('python pipelineController.py -c test/testList.json --info dicom2glb', cwd=newCwd, shell=True).decode("utf-8")
	assert "description: Generate glb mesh from dicom" in str(output)

def test_main_info_1():
	output = subprocess.check_output('python pipelineController.py -c test/testList.json --info doesNotExist', cwd=newCwd, shell=True).decode("utf-8")
